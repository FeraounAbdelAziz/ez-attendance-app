...
import { homeSharp, arrowBackCircleSharp, eyeOffSharp, eye, reader, repeat, pencil } from 'ionicons/icons';
...
const StudentPage = () => {
    ...
    const { group_name, group_id, class_id, class_name, speciality, session_id }: any = params
    ...
    const PresenteStudents = students?.filter((student: any) => student.status === 'P');
    const absentStudents = students?.filter((student: any) => student.status === 'AB');
    const absentJStudents = students?.filter((student: any) => student.status === 'ABJ');
        ...
        <React.Fragment>
            <IonContent fullscreen>
                <IonHeader>
                    <IonToolbar>
                        <IonTitle>
                            <Link to={`/class/${class_id}/${class_name}/${speciality}/${group_id}/${group_name}/session`}>
                            ...
                            Students Section {group_name}</IonTitle>
                    </IonToolbar>
                    <IonToolbar>
                        <IonTitle>
                            <h6> <IonText color={"warning"}>Presente : {PresenteCount}</IonText> |  <IonText color={"danger"}>Absent : {absentCount}</IonText> | <IonText color={"success"}>AbsentJ : {absentJCount}</IonText></h6>
                        </IonTitle>
                ...
                {students?.map((student: { student_id: any, first_name: string, second_name: string, status: string, comment: string }, index: any) => (
                    ...
                    <IonItemSliding key={student.student_id}>
                        <IonItemOptions side="start">
                            ...
                            <IonModal
                                ...
                                className={'example-modal-transfert-student'}
                                isOpen={isCommentModalOpen && selectedStudentIndex === student.student_id}
                            ...
                            <IonModal
                                ...
                                isOpen={isUpdateModalOpen && selectedStudentIndex === student.student_id}
                            ...
                            ><IonToolbar>
                                ...
                                <Formik
                                    ...
                                    onSubmit={async (values: { first_name: string, second_name: string }) => {
                                        const { first_name, second_name } = values

                                        const { data, error } = await supabase
                                            .from('student')
                                            .update({ first_name: first_name, second_name: second_name })
